...
module OAuth2StrategyTests
  ...
  module AuthorizeParamsTests
    ...
    test "should include any authorize params " \
      "passed in the :authorize_params option" do
      @options = { authorize_params: { foo: "bar", baz: "zip" } }
      assert_equal "bar", strategy.authorize_params["foo"]
      assert_equal "zip", strategy.authorize_params["baz"]
    end

    test "should include top-level options that are marked " \
      "as :authorize_options" do
  ...
  module CSRFAuthorizeParamsTests
    ...
    test "should store random state in the session when none is present " \
      "in authorize or request params" do
      assert_includes strategy.authorize_params.keys, "state"
      refute_empty strategy.authorize_params["state"]
      refute_empty strategy.session["omniauth.state"]
      assert_equal strategy.authorize_params["state"],
                   strategy.session["omniauth.state"]
    end

    test "should not store state in the session when present " \
      "in authorize params vs. a random one" do
    ...
    test "should not store state in the session when present in request " \
      "params vs. a random one" do
  ...
  module TokenParamsTests
    ...
    test "should include any authorize params passed " \
      "in the :token_params option" do
